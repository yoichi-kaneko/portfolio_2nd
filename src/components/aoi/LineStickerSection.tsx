...
/** 挿絵の枠。画像でも仮スペースでも同じ寸法・角丸を使い、差し替えでレイアウトが動かないようにする。 */
const FRAME =
  "relative aspect-[16/10] overflow-hidden rounded-[18px] border border-[rgba(127,212,255,0.22)] bg-[#0c1426]";

/**
 * LINE スタンプのセクション。
 *
 * 挿絵は後日差し替える前提のため、`STICKER_ILLUSTRATION.src` が null の間は
 * 同じ縦横比の仮スペースを描画する（差し替えはデータ側の 1 行で済む）。
 * QR コードは販売ページ URL から生成した静的画像で、画像自体も販売ページへのリンクにしている。
 */
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-[22px] lg:grid-cols-[1.32fr_0.68fr]">
        ...
        {src ? (
          ...
          <div
            data-testid="aoi-sticker-illustration-placeholder"
            className={`${FRAME} flex items-center justify-center border-dashed`}
          >
            <div className="px-[24px] text-center">
              <div className="mb-[8px] font-space text-[10px] tracking-[0.2em] text-[#6db4e6]">
                ILLUSTRATION
              </div>
              <div className="font-zen text-[18px] font-black text-[#bcd2ec]">
                挿絵は準備中
              </div>
              <p className="m-0 mt-[8px] text-[12.5px] leading-[1.8] text-[#7e90ad] text-pretty">
                スタンプの雰囲気が伝わる一枚を、あらためて描き下ろします。
              </p>
            </div>
          </div>
        )}
